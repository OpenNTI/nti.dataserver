...
def _get_shared_dataserver():
	import _Dataserver as dataserver
	return dataserver.Dataserver.get_shared_dataserver()
	...
	*Everyone*). The follows relationship is relevant only for the latter
	...
	def getContainedStream( self, containerId, minAge=0, maxCount=MAX_STREAM_SIZE ):
		...
		def add( item, lm=None ):
			...
			rlm = result.lastModified
			result.append( item )
			result.lastModified = max( rlm, lm )
...
class SharingSource(SharingTarget):
	...
	def getSharedContainer( self, containerId, defaultValue=() ):
		...
		result.extend( super(SharingSource,self).getSharedContainer( containerId, defaultValue=defaultValue ) )
		...
		for following in self._following:
			...
			if following is None: continue
			if isinstance( following, DynamicSharingTarget ):
				result.extend( [x() for x in following.getSharedContainer( containerId )
								if x() and not self.is_ignoring_shared_data_from( x().creator )] )
			else:
				persons_following.append( following )
		...
		for comm in self._communities:
			comm = self.get_entity( comm )
			if comm is None: continue
			result.extend( [x() for x in comm.getSharedContainer( containerId )
...
class ShareableMixin(datastructures.CreatedModDateTrackingObject):
	...
	def addSharingTarget( self, target, actor=None ):
		...
		if target not in self._sharingTargets:
			...
			if not self.creator or (self.creator != target and self.creator.username != target):
...
class User(Principal):
	...
	def accept_shared_data_from( self, source ):
		...
		if super(User,self).accept_shared_data_from( source ) and not already_accepting:
	...
	def addContainedObject( self, contained ):
		...
		if isinstance( result, persistent.Persistent ) \
			   and getattr( self, '_p_jar', None ) \
			   and not getattr( result, '_p_jar', None ):
	...
	def getContainer( self, containerId, defaultValue=None ):
		...
		if stored_value is defaultValue:
			...
			if not transcript_value or (len(transcript_value) == 1 and 'Last Modified' in transcript_value):
				# Neither a container nor any transcripts
				return defaultValue

			# We have no container, but we do have transcripts
			# This dict already looks like a container
			assert transcript_value.creator == self.username
			assert transcript_value.lastModified >= 0
			return transcript_value

		if not transcript_value  or (len(transcript_value) == 1 and 'Last Modified' in transcript_value):
			# No transcripts, but we do have a container
			return stored_value

		transcript_parent = location.Location()
		transcript_parent.__name__ = _TranscriptsMap.container_name
		transcript_parent.__parent__ = self
		for summary in transcript_value.itervalues():
			if not hasattr( summary, 'ContainerId' ): continue
			target = location.Location()
			target.__parent__ = transcript_parent
			target.__name__ = summary.ContainerId
			summary.links.append( links.Link( target, rel='detail' ) )

		# The complicated case is both transcripts and the container.
		# We could optimize this by delaying and doing things lazily,
		# but the simple case is to copy. Note that the container
		# uses numeric keys while the transcript uses UUID keys, so no overlap
		result = datastructures.CreatedModDateTrackingPersistentMapping( stored_value )
		result.update( transcript_value )
		result.creator = self.username
		result.lastModified = max( getattr( stored_value, 'lastModified', 0),
								   getattr( transcript_value, 'lastModified', 0) )
		return result
		...
		if self._v_updateDepth <= 0:
			...
			for possiblyUpdated in self._v_updateSet:
				...
				if updated:
					...
					self.containers[updated.containerId].updateLastMod( updated.lastModified )
	...
	def _postNotification( self, changeType, objAndOrigSharingTuple ):
		...
		if not isinstance( objAndOrigSharingTuple, tuple ):
			...
			try:
				...
				logger.debug( "Failed to get sharing targets on obj; not dist change %s", obj )
				return
		else:
