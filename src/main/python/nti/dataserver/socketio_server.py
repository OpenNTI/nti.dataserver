...
class SocketIOHandler(socketio.handler.SocketIOHandler):
	...
	# def do_handle_one_socketio_session_response( self, *args ):
	# 	with self.handling_session_cm:
	# 		super(SocketIOHandler,self).do_handle_socketio_response( *args )
	# 		except ConflictError:
	# 			tries -= 1
	# 			print 'Retrying on conflict error'
	# 			if callable( getattr( self, 'reset_passenger', None ) ):
	# 				getattr( self, 'reset_passenger' )()
	# 		else:
	# 			return
