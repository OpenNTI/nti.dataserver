...
def _read_pids_file(reverse = False):
	
	if not os.path.exists(_pids_file_name):
		return []
		
	with open(_pids_file_name, "r") as f:
		lines = f.readlines()
...
def _reset_pids_file():
	result = _read_pids_file()
	for _, ip, port in list(result):
		if not _send_message(ip, port, 'terminate'):
			result.pop(0)
...
def _save_proc_info(ip, port, args=[]):
	with open( _pids_file_name, 'a+' ) as f:
