...
# Because the stream externalizer doesn't call Mapping Decorator
...
class _StreamChangeNotableDecorator(AbstractAuthenticatedRequestAwareDecorator):
    ...
    def _is_notable(self, context):
        ...
        # it is about 100x faster than running our object through the
        # algorithm.
        result = False
        ...
        if     intids.queryId(context) is not None \
            ...
            result = is_notable_ctx(self.remoteUser) \
                  or is_notable_obj(self.remoteUser)
        return result
