...
class TestIO(AppLayerTest):
    ...
    def test_integration_note_body_validation_empty_error_message(self):
        ...
        with self.assertRaises(hexc.HTTPUnprocessableEntity) as exc:
            obj_io.update_object_from_external_object(n,
                                                      {'body': ['', '']},
                                                      request=DummyRequest())
    ...
    def test_wrong_contained_type(self):
        ...
        class IThing(interface.Interface):
            __name__ = ValidTextLine(title=u"The name")  # unicode

        @interface.implementer(IThing)
        class Thing(object):
            __name__ = b'not-unicode'

        field = UniqueIterable(value_type=Object(IThing, __name__='field'))
    ...
    def test_translate(self):
        ...
        with self.assertRaises(hexc.HTTPUnprocessableEntity) as exc:
            try:
                ...
                error.raise_json_error(DummyRequest(),
                                       hexc.HTTPUnprocessableEntity,
                                       _(u"Please provide a valid ${field}.",
                                         mapping={'field': 'email'}),
                                       None)
