...
def attach_idp_user_info(event):
	...
	if event.idp_id not in idp_user_info_container:
		...
		idp_user_info = component.queryAdapter(event.user_info, ISAMLProviderUserInfo, event.idp_id)
		if idp_user_info is not None:
			idp_user_info_container[event.idp_id] = idp_user_info
		else:
			logger.warn('Failed to adapt ISAMLUserAssertionInfo("%s") to ISAMLProviderUserInfo for user "%s", event "%s"',
						str(event.user_info),
						str(event.user.username),
						str(event))
	else:
