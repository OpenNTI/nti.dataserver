...
class DefaultEditLinkMaker(object):
    ...
    def make(self, context, unused_request=None, allow_traversable_paths=True, link_method=None):
        ...
        return _make_link_to_context(context, allow_traversable_paths, link_method)
...
class EditLinkDecorator(AbstractAuthenticatedRequestAwareDecorator):
    ...
    def _make_link_to_context(self, context):
        ...
        if maker is not None and context is not None:
            return maker.make(context, self.request,
                              self.allow_traversable_paths, self.link_method)
        return None
    ...
    def _do_decorate_external(self, context, mapping):
        ...
        # Should probably use request ILocationInfo to traverse back up to the
        # ISite
        ...
        if (needs_href or needs_edit) and not hasattr(context, 'href'):
            ...
            rendered = render_link(edit_link, nearest_site=nearest_site)
            mapping['href'] = rendered['href']
            ...
            # FIXME: Note duplication of IShouldHaveTraversablePath checks; cf
            # pyramid_renderers
...
class OIDEditOrDeleteLinkDecorator(OIDEditLinkDecorator):
    ...
    def _has_permission(self, context):
        writable = super(OIDEditOrDeleteLinkDecorator,
                         self)._has_permission(context)
        if writable:
            return writable
