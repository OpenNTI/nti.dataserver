...
def _service_path_adapter(unused_context, request):
...
class GenericGetView(AbstractView):
    ...
    def __call__(self):
        ...
        resource = getattr(self.request.context,
                           'resource', self.request.context)
        ...
        if hasattr(result, '__parent__'):
            ...
            if getattr(resource, '__parent__', None) is not None:
                ...
                if      self.request.traversed[-1] == 'Classes' \
                    and self.request.traversed[0] == 'providers':
                    result.__parent__ = self.request.context.__parent__
                elif    self.request.traversed[-1] == 'Pages' \
                    and self.request.traversed[0] == 'users':
                    result.__parent__ = self.request.context.__parent__
            elif    resource is not self.request.context \
                and hasattr(self.request.context, '__parent__'):
                result.__parent__ = self.request.context.__parent__
        return result
