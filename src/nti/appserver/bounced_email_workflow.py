...
def process_ses_feedback( messages, dataserver=None, mark_transient=True ):
	...
	:return: A tuple. The first element is a list of all the messages that were processed for bounce information.
	...
	return (proc_messages, matched_messages)
...
def process_sqs_messages():
	...
	def _proc():
		...
		proc, matched = process_ses_feedback( gen() )
		return proc, matched, fb_q

	env_dir = _get_env_dir( args.env_dir )

	_, matched_messages, fb_q = run_with_dataserver( environment_dir=env_dir,
													 function=_proc,
													 xmlconfig_packages=('nti.appserver',),
													 verbose=args.verbose )

	if asbool(args.delete) and matched_messages:
		...
		# not real-world use)
		fb_q.delete_message_batch(matched_messages)
