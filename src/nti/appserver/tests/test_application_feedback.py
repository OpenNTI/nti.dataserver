...
class TestApplicationFeedback(ApplicationLayerTest):
        ...
        def test_post_feedback_sends_email(self):
                url = '/dataserver2/users/ossmkitty/@@send-feedback'
                mail = self._do_test_email(url, 'Feedback From ossmkitty on localhost', 'body')

                # No longer expect request details in feedback emails
                assert_that(mail.body, does_not(contains_string('Details')))
                assert_that(mail.html, does_not(contains_string('Details')))
        ...
        def test_post_crash_sends_email(self):
                ...
                #Crash mail still doe have the request details
                assert_that(msg.body, contains_string('Details'))
                assert_that(msg.html, contains_string('Details'))
