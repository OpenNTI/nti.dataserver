...
class TestApplicationRating(ApplicationLayerTest):
	...
	def test_rate_note(self):
		...
		assert_that(json.loads(res.body), has_entry('Links', has_item(has_entry('rel', 'rate'))))
		...
		res = testapp.post(path + '/@@rate', data, extra_environ=self._make_extra_environ())
		# and now I'm asked to unlike
		assert_that( res.status_int, is_( 200 ) )
		assert_that(res.json_body, has_entry('Rating', 4))
		assert_that(res.json_body, has_entry('Links', has_item(has_entry('rel', 'unrate'))))
		...
		# And I can unrate
		res = testapp.delete(path + '/@@unrate', extra_environ=self._make_extra_environ())
		assert_that( res.status_int, is_( 200 ) )
		assert_that(res.json_body, is_not(has_key('Rating')))
		assert_that(res.json_body, has_entry('Links', has_item(has_entry('rel', 'rate'))))
