...
from nti.appserver.policies.site_policies import NoAvatarUploadCapabilityFilter

from nti.appserver.interfaces import IUserUpgradedEvent
from nti.appserver.interfaces import IContactEmailRecovery
...
def dispatch_content_edited_to_user_policies(content, event):
    ...
    if request is None:
        return
    # Warning !! This interacts funny with chat events. There is no current request, but
...
def veto_sharing_for_unsigned_coppa_create(content, unused_creator, unused_event):
...
def veto_sharing_for_unsigned_coppa_edit(content, unused_editor, unused_event):
    if getattr(content, 'sharingTargets', None):
        raise hexc.HTTPForbidden("Cannot share objects")
...
class CoppaUserWithoutAgreementCapabilityFilter(NoAvatarUploadCapabilityFilter):
    """
...
@interface.implementer(IUserCapabilityFilter)
class MathCountsCapabilityFilter(NoAvatarUploadCapabilityFilter):
...
REL_CHILDRENS_PRIVACY_PAGE = u'childrens-privacy'
...
#: link.
REL_GENERAL_PRIVACY_PAGE = u'content.permanent_general_privacy_page'
...
#:     editing this value.
#:
REL_CONTACT_EMAIL_SENDS_CONSENT = u'contact-email-sends-consent-request'
...
def send_consent_ack_email(user, event):
    ...
    if getattr(event.upgraded_profile, 'contact_email', None):
        _email_utils.queue_simple_html_text_email('coppa_consent_approval_email',
                                                  subject=_(u"NextThought Account Confirmation"),
                                                  recipients=[
                                                      event.upgraded_profile.contact_email],
...
class AttemptingToResendConsentEmailTooSoon(ValidationError):
    i18n_message = _(
        u"It is too soon to send another consent request email. Please try again tomorrow."
    )
    field = 'contact_email'
...
def send_consent_request_on_coppa_account(user, profile, email, request, rate_limit=False):
    ...
    attachment = Attachment(attachment_filename,
                            "application/pdf", attachment_stream)
    ...
    # In this way we can always get back to the user account using VERP. See
    # nti.mailer.
    ...
    _email_utils.queue_simple_html_text_email('coppa_consent_request_email',
                                              subject=_(
												u"Please Confirm Your Child's NextThought Account"
											  ),
                                              recipients=recip,
                                              template_args=dict(
                                                  user=user, profile=profile, context=user
                                              ),
    ...
    logger.info("Will send COPPA consent notice to %s on behalf of %s",
			    email, user.username)
    setattr(profile, 'contact_email', None)

    # We do need to keep something machine readable, though, for purposes of
    # bounces
...
def _send_consent_request(user, profile, email, event, rate_limit=False):
    if not email or not event.request:  # pragma: no cover
        return
    send_consent_request_on_coppa_account(
        user, profile, email, event.request, rate_limit
    )
...
class ContactEmailRecovery(object):
    ...
    contact_email_recovery_hash = annotation_alias(
        CONTACT_EMAIL_RECOVERY_ANNOTATION, 'context'
    )
    consent_email_last_sent = annotation_alias(
        CONSENT_EMAIL_LAST_SENT_ANNOTATION, 'context'
    )
