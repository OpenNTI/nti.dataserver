...
class ICourseInfo (interface.Interface):
	...
	duration = ValidTextLine(title='course duration', required=False)
	isPreview = Bool (title='course preview', required=False)
	instructors = ListOrTuple(value_type=Object(IInstructor), title='list of course instructors', required=True)
	prerequisites =  ListOrTuple(value_type=Object(IPrerequisite), title='course credits', required=False)
	credit  = ListOrTuple(value_type=Object(ICredit), title='course credits', required=False)
