...
from nti.contentrange.contentrange import TextDomContentPointer
...
class treewalker(object):
    ...
    def first_word(self, node):
        if node is None or node.nodeType != node.TEXT_NODE:
            # following return would raise, no 'offset' defined: #return '',
            # node, offset
            raise NotImplementedError()
...
class backward(treewalker):
    ...
    def beginning(self, a_list):
        return len(a_list)

    def end(self, unused_a_list):
        return 0

    def near_sibling(self, node):
        return node.previousSibling

    def find_space(self, string, offset):
        return string.rfind(' ', 0, offset - 1)

    def context_offset(self, node, lastspace, unused_ctx):
        return len(node.data) - lastspace
...
class forward(treewalker):
    ...
    def beginning(self, unused_list):
        return 0

    def end(self, a_list):  # pylint: disable=arguments-differ
        return len(a_list)
    ...
    def find_space(self, string, offset):
        return string.find(' ', offset + 1)

    def context_offset(self, unused_node, lastspace, unused_ctx):
        return lastspace
...
def is_anchorable(node):
    ...
    if node.nodeType == node.ELEMENT_NODE:
        ...
        if node.getAttribute('data-non-anchorable') == 'true':
            return False
        # ext-gen controversial at the moment
        illegal_starts = ('MathJax', 'ext-gen')
...
def domToContentRange(domrange):
    ...
    def element_convert(element):
        ...
        """
        return_ptr = ElementDomContentPointer()
    ...
    def point_convert(node, offset, directionals):
        ...
        return_ctx.contextOffset = directionals.context_offset(node, lastspace,
                                                               return_ctx)
        # Append additional contexts until 15 characters or 5 secondary
        # contexts
        return_ptr = TextDomContentPointer()
    ...
    output = DomContentRangeDescription()
...
def contentToDomRange(contentrange, document):
    def seek_dom_element(ancestor, ptr):
        if      ancestor.nodeType == ancestor.ELEMENT_NODE \
            and ancestor.getAttribute('id') == ptr.elementId \
            and ancestor.tagName == ptr.elementTagName:
            return ancestor
    ...
    def point_convert(point, start_limit=None):
        if hasattr(point, 'elementId') and hasattr(point, 'elementTagName'):
            ...
            treewalk = {"start": backward(), "end": forward()}[
                point.role].onestep
            ...
            def get_matches(node, point, start_limit=None):
                if node.nodeType == node.TEXT_NODE:
                    ...
                    while 1:
                        left = node.data.find(
                            point.contexts[0].contextText, left + 1)
                        if left == -1:
                            break
                        f = math.sqrt(len(node.data)) * 2 + 1.0
                        if point.role == "start":
                            offset = len(node.data) - \
                                point.contexts[0].contextOffset
                        else:
                    ...
                    for i, cxt in enumerate(point.contexts[1:]):
                        ...
                        if cur_node is None:
                            failed = True
                        elif    cur_node.nodeType == cur_node.TEXT_NODE \
                            and cur_node.data.find(cxt.contextText) == -1:
                            failed = True
                        elif    cur_node.nodeType == cur_node.ELEMENT_NODE \
                            and cxt.contextText != '':
                            failed = True
                        if failed:
                    ...
                    totalchars = sum([len(c.contextText)
                                      for c in point.contexts[1:]])
                    if score_multiplier == 1 and nsc < 5 and totalchars < 15:
                        if cur_node is not None:
                            score_multiplier = (nsc + 1) / (nsc + 1.5)
                    outputs = [
                        (_domrange.position(node, o[0] + point.edgeOffset), o[1] * score_multiplier)
                        for o in allmatches
                    ]
                    if start_limit is None:
                        return outputs
                    return [x for x in outputs if x[0] > start_limit]
                else:
                    child_results = []
                    sl = start_limit
                    ...
                    for c in node.childNodes:
                        ...
                        if a is not None:
                            child_results.extend(a)
                    return child_results
                return []

            all_matches = sorted(get_matches(ancestor, point, start_limit),
                                 key=lambda x: x[1])
