...
def updating(f):
    # Call parent range's update function after 
    # setting start or end nodes
    def inner_func(*args, **kwargs):
        f(*args, **kwargs)
        if args[0].parent is not None:
            args[0].parent.update()
    return inner_func
...
class Range(object):
    ...
    def update(self):
        if self.start.node is not None and self.end.node is not None:
            self.ancestor = find_common_ancestor(self.start.node, 
                                                 self.end.node)
    ...
    def set_start_before(self, node): 
        self.start.set_before(node)

    def set_start_after(self, node): 
        self.start.set_after(node)

    def set_end_before(self, node): 
        self.end.set_before(node)

    def set_end_after(self, node): 
        self.end.set_after(node)

    def get_root(self): 
        return get_root(self.ancestor)
