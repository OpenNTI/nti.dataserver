...
class DomTests(unittest.TestCase):
    ...
    def setUp(self):
        # Preserve Tabs in string. XXX
        self.doc = test_helpers.Document(u"""
		<table id="thetable">
			<tr id="firstrow">
				<td>
					<p>123</p>
		...
		""")
        self.el = {
            s: self.doc.text_hunt(s) for s in ['456', 'a', 'b', 'c', '123456789']
        }
        self.el1 = self.el['456'].parentNode
        self.el2 = self.doc.downpath([1, 0])
        self.r = [
            _domrange.Range(self.el['456'], 2, self.el2, 0),
            _domrange.Range(),
    ...
    def test_point_comparisons(self):
        ...
        for c in comparisons:
            test_helpers.point_in_range_check(*c)

    def test_range_comparisons(self):
        r = self.r
        # First starts before second
        comparisons = [
            (r[0], r[2], self.START_TO_START, self.BEFORE),
            # Two synonyms
        ...
        for c in comparisons:
            test_helpers.range_comparison_check(*c)
