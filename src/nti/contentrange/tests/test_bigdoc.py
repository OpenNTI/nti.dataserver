...
resource_string = getattr(pkg_resources, 'resource_string')


class BigDocTests(unittest.TestCase):

    def setUp(self):
        self.main = resource_string('nti.contentrange.tests', 'bigdoc.html')
        self.main = u"".join(i for i in self.main if ord(i) < 128)
        self.main = re.sub(r'\&\#8[0-9][0-9][0-9]', ' ', self.main)
    ...
    def test1(self):
        ...
        n3, o3 = bigdoc.text_hunt("selections/ranges", 
								  offset=True, querypos=10)
        n4, o4 = bigdoc.text_hunt("specific ranges of con", 
								  offset=True, querypos=8)
        n5, o5 = bigdoc.text_hunt("effect. This", offset=True, querypos=-5)
        n6, o6 = bigdoc.text_hunt("outside", offset=True, 
                                  querypos=6)  # 1st occurrence
        n7, o7 = bigdoc.text_hunt("Dom Range Specification", bigdoc.downpath([7]),
                                   True, 9)
        ...
        # Main context in ct[4] will be blank, secondaries will be the
        # distinguishing factor
        assert_that(True in ["class" in c.contextText for c in ct[4].end.contexts], 
                    is_(True))
