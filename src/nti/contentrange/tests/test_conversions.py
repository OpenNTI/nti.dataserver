...
class ConversionTests(unittest.TestCase):
    ...
    def setUp(self):
        self.doc1 = test_helpers.Document(u"""
		<p id="id">
			A single selected text node
		</p>
		""")
        self.doc2 = test_helpers.Document(u"""
		<p id="id">
			An<i> italic </i>word.
		</p>
		""")
        self.doc3 = test_helpers.Document(u"""
		<p id="id">
			This is the <i>first<img id="123"></img></i> sentence.
			<span>This is the<i> second </i>sentence.</span>
		</p>
		""")
        self.doc4 = test_helpers.Document(u"""
		<body>
        ...
        self.ranges = [
			_domrange.Range(self.el1, 0, self.el1, 27),
    ...
    def dom_to_contentrange_tests(self):
        self.solutions = [
			[u'A', 27, u'node', 23, u'id', None],
			[u'An', 2, u'word.', 0, u'id', None],
			[u'is the', 6, u'sentence.', 0, u'id', None],
			[None, u'img'],
			[None, None, None, None, u"ThisIdIsTheBest", None]
		]
        self.cr = test_helpers.range_conversion_check(self.ranges, 
													  self.solutions)

    def contentrange_to_dom_tests(self):
        self.bk_solutions = [
			u'A single selected text node',
			u'An italic word.',
			u'the second sentence.',
			self.NO_CHECK
		]
        cr, _ = test_helpers.round_trip_check(self.ranges, None, 
											  self.bk_solutions)
        _ = contentrange.DomContentRangeDescription()
        cpbroken = cr[3]
        cpbroken.start.elementId = u"wegrhstrytjh"
        bkbroken = _convertrange.contentToDomRange(cpbroken, 
												   self.ranges[3].get_root())

        assert_that(str(bkbroken), is_('None'))

    def anchorability_tests(self):
        doc = test_helpers.Document(u"""
		<body>
			<div id="123">
        ...
        for x, y in zip(range(6), [True, False, False, True, False, False]):
            assert_that(_convertrange.is_anchorable(doc.downpath([0, x])), 
						is_(y))
        assert_that(_convertrange.is_anchorable(doc.root), is_(False))
        assert_that(_convertrange.is_anchorable(None), is_(False))

        doc = test_helpers.Document(u"<body>123456<img></img></body>")
        assert_that(_convertrange.is_anchorable(doc.root.childNodes[0]), 
					is_(False))
        assert_that(_convertrange.is_anchorable(doc.root.childNodes[1]), 
					is_(False))
    ...
    def tests_copied_from_javascript(self):
        ...
        r = _domrange.Range(doc.downpath([0, 0, 0]), 3, 
						  	doc.downpath([0, 0, 0]), 6)
    ...
    def integration_tests(self):
        ...
        r = _domrange.Range(doc.downpath([0, 0, 0]), 0, 
						  				 doc.downpath([0, 1, 0]), 18)
        ...
        r = _domrange.Range(doc.root.childNodes[1], 0, 
						    doc.root.childNodes[3], 0)
    ...
    def shifting_document_tests(self):
        ...
        # in the documents.
        original_doc = test_helpers.Document(u"""
		<div id="123">
			<p id="45">
        ...
        r = (
			_domrange.Range(p[0], 13, p[0], 47),
			_domrange.Range(p[0], 13, p[1], 22)
		)
        new_doc1 = test_helpers.Document(u"""
		<div id="123">
			<p id="intruder">Ugh. </p>
			<p id="45">
		...
		""")
        sln1 = [
			u"somewhat but probably not particularly long",
            u"somewhat but probably not particularly long text for readers"
            u" with short attention spans. Here are some extra words. This"
            u" is some more text"
        ]
        ...
        new_doc2 = test_helpers.Document(u"""
		<div id="123">
			<p id="intruder">Ugh. </p>
			<p id="45">
        ...
        sln2 = [
			u"somewhat but probably not particularly long",
			u"somewhat but probably not particularly long text for readers"
			u" with short attention spans. Here are some extra words in a"
			u" span. This is some more text"
		]
