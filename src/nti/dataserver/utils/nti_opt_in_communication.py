...
def _get_user_info(user):
	...
	is_copaWithAgg = nti_interfaces.ICoppaUserWithAgreementUpgraded.providedBy(user)
...
def _seek_users(useindex=False, verbose=False, stream=sys.stdout):
	
	if verbose:
		print('\t'.join(_fields), file=stream)
	else:
		print('\t'.join(_fields[:-1]), file=stream)
	...
	for user in users:
		...
		if verbose:
			print('\t'.join(out_t), file=stream)
		elif useindex or info.opt_in:
			print('\t'.join(out_t[:-1]), file=stream)
