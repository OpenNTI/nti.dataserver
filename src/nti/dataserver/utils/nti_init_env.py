...
def main():
    arg_parser = argparse.ArgumentParser(description="Initialize a base dataserver "
                                         "environment")
    arg_parser.add_argument('pserve_ini_file',
                            help="The path to the .ini file for pserve")
...
def init_env(args):
    ...
    if args.verbose:
        level = logging.WARN if not args.verbose else logging.INFO
        logging.basicConfig(level=level)

        fmt = zope.exceptions.log.Formatter('[%(name)s] %(levelname)s: %(message)s')
        logging.root.handlers[0].setFormatter(fmt)

    if args.with_example:
        # If we do this, we must provide a password utility or we cannot create
        # users
        component.provideUtility(example_database_initializer.ExampleDatabaseInitializer(),
