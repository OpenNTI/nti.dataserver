...
def add_remove_friends(owner, name, add_members=(), remove_members=()):
    thelist = None
    flname = name.lower()
    for fl in getattr(owner, 'getFriendsLists', lambda unused: ())(owner):
        if not IEntity.providedBy(fl):
            continue
        username = fl.username.lower()
        realname = IFriendlyNamed(fl).realname or ''
    ...
    data =  { 'friends': list(final_friends)}
    result = update_from_external_object(thelist, data)
    return result
