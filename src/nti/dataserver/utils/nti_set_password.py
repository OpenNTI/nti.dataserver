...
def _set_pass(username, password):
    ...
    if not user or not IUser.providedBy(user):
        ...
        sys.exit(2)
    user.password = text_(password)


def main():
    arg_parser = argparse.ArgumentParser(description="Interactively change the password "
                                         "of an existing user account")

    arg_parser.add_argument('username',
                            help="The username whose password to change")
