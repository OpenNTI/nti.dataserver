...
HTTP_HEADER_LAST_MODIFIED = httpheaders.LAST_MODIFIED.name
from datastructures import StandardExternalFields
...
class Get(object):
	...
	@property
	def users(self):
		return component.getUtility(nti_interfaces.IDataserver).root['users']
	...
	def __makeHeadersExternal( self, headers ):
		if isinstance( headers.get(HTTP_HEADER_LAST_MODIFIED,None), (float,int) ):
			headers[HTTP_HEADER_LAST_MODIFIED] = wsgiref.handlers.format_date_time( headers[HTTP_HEADER_LAST_MODIFIED] )
	...
	def respond( self, body, environ, start_response, wrap_group=None, status='200 OK', addlHeaders=None):
		...
		if body is None:
			...
			if wrap_group:
				...
				bodyWrapper[StandardExternalFields.LAST_MODIFIED] = self.findLastModified( body )
			...
			if not wrap_group \
				   and StandardExternalFields.LAST_MODIFIED not in body \
				   and HTTP_HEADER_LAST_MODIFIED not in headers:
				lastMod = self.findLastModified( body )
				if lastMod > 0:
					headers[HTTP_HEADER_LAST_MODIFIED] = lastMod

			if StandardExternalFields.LAST_MODIFIED in body \
				   and HTTP_HEADER_LAST_MODIFIED not in headers \
				   and isinstance(headers,collections.Mapping) :
				headers[HTTP_HEADER_LAST_MODIFIED] = body[StandardExternalFields.LAST_MODIFIED]
				self.__makeHeadersExternal( headers )

			if HTTP_HEADER_LAST_MODIFIED in headers and status.startswith( '200' ) and ifLastModified( environ ) :
				...
				lm = time.mktime( email.utils.parsedate( headers[HTTP_HEADER_LAST_MODIFIED] ) )
			...
			if self.findFormat( environ ) == 'json':
				...
				body = to_external_representation( body, EXT_FORMAT_JSON, name='wsgi' )
			else:
				contentType = 'application/xml'
				body = to_external_representation( body, EXT_FORMAT_PLIST, name='wsgi' )
	...
	def __call__(self, environ, start_response ):
		with NoOpCM():
			...
			if self.getKey( environ ) not in parentDict:
				...
				lastModified = self.updateParentLastMod( environ, parentDict[StandardExternalFields.LAST_MODIFIED] )
		return self.respond( None, environ, start_response, False, status, {HTTP_HEADER_LAST_MODIFIED: lastModified } )
