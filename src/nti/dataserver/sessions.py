...
class SessionService(object):
		...
		def watchdog_sessions():
			while True:
				...
				try:
					...
					# Happens if startup takes too long, e.g., while downloading index data
	...
	def get_session_by_owner(self, session_owner_name, session_ids=_AlwaysIn()):
		...
		elif not isinstance(session_ids, _AlwaysIn) and \
			 isinstance(session_ids, (basestring, numbers.Number)):
...
def _send_notification(user_notification_event):
	...
	"""
	sessions = getattr(component.queryUtility(IDataserver), 'session_manager', None)
	if sessions:
