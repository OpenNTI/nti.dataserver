...
class _AbstractMeetingContainerAdapter(object):
    ...
    def create_meeting_from_dict(self, chatserver, meeting_dict, constructor):
        active_meeting = self._has_active_meeting()
        if active_meeting:
            logger.debug("Not creating new meeting due to active meeting %s", 
                         active_meeting)
            # veto creation
            return None
        ...
        orig_occupants = {
            (x[0] if isinstance(x, tuple) else x) : x for x in meeting_dict['Occupants']
        }

        occupants = [(o if o not in orig_occupants else orig_occupants[o])
                     for o in occupants]
        meeting_dict['Occupants'] = occupants

        result = constructor()
        # ant_interfaces.IAnnotations( self.context )[self.ACTIVE_ROOM_ATTR] = result
        self._active_meeting = result
        ...
        # the object is not configured yet, so for now we simply match its
        # policy
...
class _FriendsListAdapter(_AbstractMeetingContainerAdapter):
    ...
    def enter_active_meeting(self, chatserver, meeting_dict):
        ...
        """
        active_meeting = super(_FriendsListAdapter, self).enter_active_meeting(
            chatserver, meeting_dict)
        if active_meeting and meeting_dict.get('Creator') == self.friends_list.creator.username:
            logger.debug("Recreating friends list room for creator %s (due to missing people %s != %s?)",
                         meeting_dict.get('Creator'), 
                         active_meeting.occupant_names, 
                         self._allowed_occupants)
