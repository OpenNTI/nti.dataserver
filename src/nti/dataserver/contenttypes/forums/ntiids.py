...
@interface.implementer(nid_interfaces.INTIIDResolver)
class _ClassBoardResolver(AbstractAdaptingUserBasedResolver):
	"""
	Resolves the class board that belongs to a user, if one does exist.

	Register with the name :const:`.NTIID_TYPE_CLASS_BOARD`
	"""

	required_iface = nti_interfaces.IUser
	adapt_to = frm_interfaces.IClassBoard

@interface.implementer(nid_interfaces.INTIIDResolver)
class _ClassForumResolver(AbstractMappingAdaptingUserBasedResolver):
	"""
	Resolves a forum that belongs to a class.

	Register with the name :const:`.NTIID_TYPE_CLASS_FORUM`
	"""

	required_iface = nti_interfaces.IUser
	adapt_to = frm_interfaces.IClassBoard  # adapt to a board, look inside for a named forum

	def _resolve(self, ntiid, user):
		forum = super(_ClassForumResolver, self)._resolve(ntiid, user)
		return forum

@interface.implementer(nid_interfaces.INTIIDResolver)
class _ClassTopicResolver(AbstractUserBasedResolver):
	"""
	Resolves a topic in the one forum that belongs to a class, if one does exist.

	Register with the name :const:`.NTIID_TYPE_CLASS_TOPIC`
	"""

	required_iface = nti_interfaces.IUser
	adapt_to = frm_interfaces.IClassForum

	def _resolve(self, ntiid, user):
		board = frm_interfaces.IClassBoard(user, None)
		if board is None:
			return None
		forum_name, topic_name = get_specific(ntiid).split('.', 1)
		return board.get(forum_name, {}).get(topic_name)
