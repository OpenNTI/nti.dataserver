...
class ReferenceObject(WeakRef):
    ...
    __slots__ = ('external_ntiid_oid', 'ref_is_deleted', 'creator',
                 'sharingTargets', 'references', 'inReplyTo', 'replies', 'referents')
...
class TestThreadable(DataserverLayerTest):
    ...
    def test_write_external_reply_to_deleted(self):
        ...
        assert_that(ext, 
                    has_entry('inReplyTo', 'tag:nextthought.com,2011-10:Missing-x'))
        assert_that(ext, 
                    has_entry('references', ['tag:nextthought.com,2011-10:Missing-y']))

        assert_that(note, 
                    validly_provides(IInspectableWeakThreadable))
    ...
    def test_adding_and_removing_maintains_reply_chains(self):
        ...
        assert_that(list(root.replies), is_([direct_reply]))
        assert_that(list(root.referents), 
                    contains_inanyorder(direct_reply, direct_reply_child))

        grandchild2 = Note()
        grandchild2.inReplyTo = direct_reply
        container['grandchild2'] = grandchild2

        assert_that(list(direct_reply.replies), 
                    contains_inanyorder(direct_reply_child, grandchild2))

        assert_that(list(direct_reply.referents),
                    contains_inanyorder(direct_reply_child, grandchild2))

        assert_that(list(root.replies), is_([direct_reply]))
        assert_that(list(root.referents), 
                    contains_inanyorder(direct_reply, direct_reply_child, grandchild2))
        ...
        assert_that(list(root.replies), is_([direct_reply]))
        assert_that(list(root.referents), 
                    contains_inanyorder(direct_reply, grandchild2))
