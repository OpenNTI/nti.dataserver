...
class FriendsList(SimpleEnclosureMixin, Entity):  # Mixin order matters for __setstate__
    ...
    def _on_added_friend(self, friend):
        ...
        if callable(getattr(friend, 'accept_shared_data_from', None)):
            ...
            # TODO: used to be an instance check on SharingSource
    ...
    def _creator_username(self):
        # for purposes of caching NTIID
        return self.creator.username if self.creator else 'Unknown'

    # Actually those really shouldn't change, right?
    @CachedProperty('_creator_username', 'username')
    def NTIID(self):
    ...
    containerId = property(lambda self: 'FriendsLists',
                           lambda self, unused_nv: None)  # Ignore attempts to set
    ...
    def updateFromExternalObject(self, parsed, *args, **kwargs):
        ...
        if newFriends is not None:
            ...
            if updated:
                ...
                # Sigh. Some general_purpose tests depend on this, regardless
                # of whether any update got done.
                self.updateLastMod()
    ...
    def _resolve_friends(cls, dataserver, parsed, externalFriends):
        ...
        for externalFriend in externalFriends:
            result.append(cls.get_entity(externalFriend,
                                         dataserver=dataserver, default=externalFriend))
        return result
    ...
    def __eq__(self, other):
        ...
        if result is True:
            try:
                result = self.creator == other.creator \
...
class DynamicFriendsList(DynamicSharingTargetMixin, FriendsList):  # order matters
    ...
    def updateFromExternalObject(self, parsed, *args, **kwargs):
        ...
        updated = super(DynamicFriendsList, self).updateFromExternalObject(
            parsed, *args, **kwargs) or updated
        return updated
...
class _DynamicFriendsListEntityIterable(_FriendsListEntityIterable):
    ...
    def __contains__(self, other):
        return super(_DynamicFriendsListEntityIterable, self).__contains__(other) \
               or other == self.context.creator
...
class _FriendsListMap(AbstractCaseInsensitiveNamedLastModifiedBTreeContainer):
    ...
    def external_factory(cls, extDict):
        ...
        result = factory(extDict['Username']
                         if 'Username' in extDict else extDict['ID'])
