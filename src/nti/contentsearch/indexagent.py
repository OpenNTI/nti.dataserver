...
def _handle_event(indexmanager, entity, changeType, change_object, broadcast=None):
	...
	entity = Entity.get_entity(entity) \
			 if isinstance(entity, six.string_types) else entity
	...
	if should_process:
		...
		result = _process_event(indexmanager, entity, changeType, data_type,
								change_object)
	return result
...
def handle_index_event(indexmanager, target, change, broadcast=None):
	...
	:param broadcast: This is set to true when events are being sent for the sole
		...
		restrictions and do process the event. NOTE: This is unfortunately tightly
		coupled with users.py and chat_transcripts.py and needs rethought.
		chat_transcripts.py sends all
