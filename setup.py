...
setup(
	...
	setup_requires=[
		...
		# when that changes
		# 'distribute >= 0.6.48', # Can't seem to include that anywhere
	...
	install_requires=[
		...
		'pyquery >= 1.2.4',
		'pyramid >= 1.4.2' ,
		'pyramid_mailer >= 0.12',  # Which uses repoze.sendmail
		'pyramid_who >= 0.3',
		'pyramid_zcml >= 1.0.0',
	...
	extras_require={
		...
		'tools': [
			...
			'httpie >= 0.6.0',  # 0.4.0 explicitly requires requests > 1.0.4, 0.3.1 explicitly requires requests < 1.0
			...
			'pudb >= 2013.3.2',  # Python full screen console debugger. Beats ipython's: import pudb; pdb.set_trace()
